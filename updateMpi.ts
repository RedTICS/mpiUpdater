                                ...
                                existeEnMpi(data, coleccion)
                                    .then((resultado => {
                                            ...
                                            if (resultado[0] !== 'merge') {
                                                ...
                                                post.cargarUnPacienteMpi(resultado[1])
                                                    .then((rta) => {
                                                        console.log('Paciente Guardado es:', resultado[1]);
                                                    });
                                            } else {
                                                /*Se fusionan los pacientes*/
                                                console.log('entro por merge se va a fusionar: ', resultado[1]);
                                                let pacFusionar = resultado[1];
                                                let idPacMpi = resultado[1]._id;
                                                console.log('id del paciente original a updetear: ', idPacMpi);
                                                db.collection(coleccion).update(
                                                    {'_id': idPacMpi},
                                                    {
                                                       $addToSet: {
                                                            ...
                                                            'direccion': {
                                                                $each: pacFusionar.direccion
                                                            },
                                                               'contacto': {
                                                                $each: pacFusionar.contacto
                                                            },
                                                               'relaciones': {
                                                                $each: pacFusionar.relaciones
                                                            }
                                                        },
                                                    },
                                                {upsert: true},
